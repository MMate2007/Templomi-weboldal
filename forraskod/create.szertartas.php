...
<div class="head-text">
<h1>Példa plébánia honlapja - Litrugia hozzáadása...</h1>
</div>
</div>
</div>
<hr>
...
$mysql = mysqli_connect("localhost", "mysqlfelhasznalo", "mysqljelszo", "adatbazisnev") or die ("<p class='warning'>A következő hiba lépett fel a MySQL-ben: ".mysqli_error($mysql)."</p>");
mysqli_query($mysql, "SET NAMES utf8");
$sql = "SELECT `id` FROM `szertartasok`";
$id = 0;
$eredmeny = mysqli_query($mysql, $sql) or die ("<p class='warning'>A következő hiba lépett fel a MySQL-ben: ".mysqli_error($mysql)."</p>");
while ($row = mysqli_fetch_array($eredmeny))
{
...
$sql = "INSERT INTO `szertartasok`(`id`, `date`, `name`, `place`) VALUES ('".$id."','".$date."','".$name."','".$place."')";
$eredmeny = mysqli_query($mysql, $sql) or die ("<p class='warning'>A következő hiba lépett fel a MySQL-ben: ".mysqli_error($mysql)."</p>");
mysqli_close($mysql);
?>
<div class="content">
